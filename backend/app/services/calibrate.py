...
def _detect_card(contours: list[np.ndarray]) -> Reference | None:
...
def _detect_coin(image: np.ndarray) -> Reference | None:
    gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
...
def _detect_reference(image: np.ndarray, contours: list[np.ndarray]) -> Reference | None:
    card = _detect_card(contours)
    if card is not None and card.confidence >= CARD_CONFIDENCE_THRESHOLD:
        return card
    return _detect_coin(image)
...
def measure(image_bytes: bytes) -> MeasurementResult:
    ...
    reference = _detect_reference(image, contours)
